...
def conferencia(driver, row_data, codigos_documento):
    """
    ...
    Returns:
        True se o encerramento foi bem-sucedido, False caso contrário
    """
    try:
...
def dados_bancarios(driver, cpf_responsavel, banco, agencia, conta, dv):
    """
    ...
    try:
        ...
        # Avança para a tela de dados bancários
    ...
    for idx, row in df.iterrows():
        ...
        if documentos_disponiveis:
            ...
            if selecionar_documento(driver, num_doc):
                if preencher_dados(driver, row[colunas_codigos], codigos_doc):
                    # Adiciona verificação de encerramento após preencher os dados
                    if conferencia(driver, row[colunas_codigos], codigos_doc):
                        # Adiciona o preenchimento de dados bancários
                        if dados_bancarios(driver, cpf_responsavel):
                            # Adiciona o envio do documento
                            if enviar_documento(driver):
        ...
        for doc_idx, doc_num in enumerate(documentos_restantes):
            ...
            if selecionar_documento(driver, doc_num):
                if preencher_dados(driver, row[colunas_codigos], codigos_doc):
                    ...
                    if conferencia(driver, row[colunas_codigos], codigos_doc):
                        # Adiciona o preenchimento de dados bancários
                        if dados_bancarios(driver, cpf_responsavel, banco, agencia, conta, dv):
                            # Adiciona o envio do documento
                            if enviar_documento(driver):
